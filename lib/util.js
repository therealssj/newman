...
util = {
    ...
    /**
     * Helper function to test if a given proxy is working for a url.
     *
     * @param {String} proxy - The proxy to test.
     * @param {string} destination - The destination url to test for.
     * @returns {Boolean}
     */
    isReachable: function (proxy, destination) {
        var parsedProxy = new url.parse(proxy),
            parsedDest = new url.parse(destination),
            auth = (parsedProxy.username && parsedProxy.password) &&
                'Basic ' + Buffer.from(parsedProxy.username + ':' + parsedProxy.password)
                    .toString('base64');
        ...
        http.request({
            ...
            method: 'CONNECT',
            path: parsedDest.protocol === 'http' ? parsedDest.hostname : parsedDest.hostname + ':443',
            headers: (auth !== undefined) ? { 'Proxy-Authorization': auth } : {}
        }).on('connect', (res) => {
            return res.statusCode === 200; // connected to proxy server
        }).on('error', () => {
            return false;
        }).end();
