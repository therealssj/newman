var _ = require('lodash'),
    ...
    configLoaders = {
        ...
        /**
         * The proxyList parsing module, with support for JSON files.
         *
         * @param {String|Object[]} location - The path to the proxy list file for the current collection run.
         * @param {Object} options - The set of wrapped options.
         * @param {Function} callback - The function invoked to indicate the end of the proxy list parsing routine.
         * @returns {*}
         */
        proxyList: function (location, options, callback) {
            if (Array.isArray(location)) {
                return callback(null, location);
            }

            if (typeof location !== 'string') {
                return callback(new Error('path for proxy list file must be a string'));
            }

            fs.readFile(location, function (err, value) {
                if (err) {
                    return callback(new Error(`unable to read the proxy list file "${location}"`));
                }
                ...
                // ensure that `proxyList` is an array
                if (!Array.isArray(value)) {
                    return callback(new Error('expected proxy list to be an array.'));
                }
        ...
        /**
         * The PAC file parsing module.
         *
         * @param {String|Object[]} location - The path to the PAC file for the current collection run.
         * @param {Object} options - The set of wrapped options.
         * @param {Function} callback - The function invoked to indicate the end of the PAC file parsing routine.
         * @returns {*}
         */
        pac: function (location, options, callback) {
            if (_.isFunction(location)) {
                return callback(null, location);
            }

            if (typeof location !== 'string') {
                return callback(new Error('path for PAC file must be a string'));
            }

            fs.readFile(location, function (err, value) {
                if (err) {
                    return callback(new Error(`unable to read the PAC file "${location}"`));
                }

                try {
                    // parse the pac file
                    // returns a function which takes in a url and outputs a proxy or a list of proxies for it.
                    value = pac(value.toString(util.detectEncoding(value)).trim());
                }
                catch (e) {
                    return callback(new Error(`the file at ${location} is not a valid PAC file.`));
                }

                var systemProxy = function (url, cb) {
                    value(url).then((res) => {
                        // split the result from the PAC function (PROXYTYPE PROXY;) into type and target
                        var [type, target] = res.split(/\s+/),
                            proxy = type === 'PROXY' || type === 'HTTP' || type === 'HTTPS' ? target : '',
                            proxyConfig, proxies, i, parsedProxy;

                        if (proxy !== '') {
                            // proxy can be a single proxy or a list of proxies like: 123.proxy.com; 234.proxy.com
                            // split it into a list and iterate over it, stop at first working proxy
                            ...
                            for (i = 0; i < proxies.length; i += 1) {
                                if (util.isReachable(proxies[i], url)) {
                                    parsedProxy = new sdk.Url(proxies[i]);
                                    proxyConfig = {
                                        match: sdk.UrlMatchPattern.MATCH_ALL_URLS,
                                        host: parsedProxy.getHost(),
                                        port: parsedProxy.port,
                                        authenticate: parsedProxy.auth && true,
                                        username: parsedProxy.auth && parsedProxy.auth.user,
                                        password: parsedProxy.auth && parsedProxy.auth.password
                                    };

                                    // break out of loop if a valid proxy is found
                                    break;
                                }
                            }
                        }

                        return cb(null, proxyConfig);
                    });
                };

                return callback(null, systemProxy);
            });
        }
    };
