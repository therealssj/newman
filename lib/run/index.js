...
module.exports = function (options, callback) {
    ...
    getOptions(options, function (err, options) {
        ...
        var sslClientCertList = options.sslClientCertList || [],
            // use proxy list to allow different proxies for different URLs
        ...
        // if proxy option is set, put it at the end of the list to
        // match all URLs that didn't match the list
        if (options.proxy) {
            const parsedProxy = new sdk.Url(options.proxy);
        ...
        runner.run(options.collection, {
            ...
            proxies: proxyList.length && new sdk.ProxyConfigList({}, proxyList),
            systemProxy: options.pac
        }, function (err, run) {
